...
class TransformerModel(nn.Layer):
    ...
    def __init__(self, ntoken, hidden, nhead, nlayers, dropout, mode='finetune', name='d_encoder'):
        super().__init__()
        print('Transformer is used for {}'.format(mode))
        self.pos_encoder = PositionalEncoding(hidden)
        # encoder_layers = TransformerEncoderLayer(hidden, nhead, hidden, dropout, normalize_before=True, activation='gelu')  # !
        ...
        self.weight_attr1 = paddle.framework.ParamAttr(name=f"{name}_linear_weight1", initializer=paddle.nn.initializer.XavierUniform())
        self.bias_attr1 = paddle.framework.ParamAttr(name=f"{name}linear_bias1", initializer=paddle.nn.initializer.XavierUniform())
        self.weight_attr2 = paddle.framework.ParamAttr(name=f"{name}linear_weight2", initializer=paddle.nn.initializer.XavierUniform())
        self.bias_attr2 = paddle.framework.ParamAttr(name=f"{name}linear_bias2", initializer=paddle.nn.initializer.XavierUniform())
        self.weight_attr3 = paddle.framework.ParamAttr(name=f"{name}linear_weight3", initializer=paddle.nn.initializer.XavierUniform())
        self.bias_attr3 = paddle.framework.ParamAttr(name=f"{name}linear_bias3", initializer=paddle.nn.initializer.XavierUniform())
        self.weight_attr4 = paddle.framework.ParamAttr(name=f"{name}linear_weight4", initializer=paddle.nn.initializer.XavierUniform())
        self.bias_attr4 = paddle.framework.ParamAttr(name=f"{name}linear_bias4", initializer=paddle.nn.initializer.XavierUniform())
        ...
        if mode == 'pretrain':
            ...
            self.fc1 = nn.Linear(hidden, 512, weight_attr=self.weight_attr1, bias_attr=self.bias_attr1)
            self.fc2 = nn.Linear(512, 256, weight_attr=self.weight_attr2, bias_attr=self.bias_attr2)
            self.fc3 = nn.Linear(256, 128, weight_attr=self.weight_attr3, bias_attr=self.bias_attr3)
            self.fc4 = nn.Linear(128, 1, weight_attr=self.weight_attr4, bias_attr=self.bias_attr4)  # !
        
    def forward(self, src, src_segment, src_padding_mask=None, mlm_label=None, generate_vec=False):
        # import ipdb
        # ipdb.set_trace()
        ...
        if self.mode == 'pretrain':  # for train
            ...
            if self.training:
                ...
                mlm_loss = F.cross_entropy(logits,  # shape=[bs, num_class, seq_len]\
                                            paddle.to_tensor(mlm_label, paddle.int64), ignore_index=config._PAD_ )
