...
def train(x_train, y_train, q_train, x_test, y_test, q_test, model_save_path):
    '''
    模型的训练和保存
    :param x_train:
    :param y_train:
    ...
    valid_data = lgb.Dataset(x_test, label=y_test, group=q_test)
    params = {
        'task': 'train',  # 执行的任务类型
        'boosting_type': 'gbdt',  # 基学习器  ##可以尝试gbdt dart goss rf
        'objective': 'lambdarank',  # 排序任务(目标函数) rank_xendcg lambdarank
        ...
        'num_iterations': 1000,  # 迭代次数，即生成的树的棵数
        'learning_rate': 0.05,  # 学习率 [0.01, 0.015, 0.025, 0.05, 0.1],一般先选一个较小的学习速率，再按情况逐步增大
        'num_leaves': 40,  # 叶子数 此参数的数值应该小于 2max_depth2^{max\_depth}2max_depth
        # 'max_depth':6,
    ...
    gbm = lgb.train(params, train_data, valid_sets=valid_data,early_stopping_rounds=50,evals_result=evals_result,feval=dcg_10) # 这里valid_sets可同时加入train_data，val_data
    # evals_result=evals_result,feval=dcg_10
    print('Save model...') 
...
def test_data_ndcg(model_path, test_path):
    ...
    gbm = lgb.Booster(model_file=model_path)
    test_predict = gbm.predict(test_X,num_iteration=gbm.best_iteration)

    average_ndcg, _ = validate(test_qids, test_y, test_predict, 10)
    # 所有qid的平均ndcg
...
if __name__ == '__main__':
    ...
    raw_data_path = train_path + 'finetune_train_w_vaild.txt'
    data_feats = train_path + 'feats.txt'
    data_group = train_path + 'group.txt'
    
    valid_path = base_path + '/data/valid/'
    raw_valid_data_path = valid_path + 'valid_f.txt'
    valid_data_feats = valid_path + 'feats.txt'
    valid_data_group = valid_path + 'group.txt'



    model_path = base_path + '/data/model/model_final.mod'
    ...
    if sys.argv[1] == '-process':
        ...
        process_data_format(raw_valid_data_path, valid_data_feats, valid_data_group)

    elif sys.argv[1] == '-train':
        # train
        train_start = datetime.now()
        x_train, y_train, q_train = load_data(data_feats, data_group)
        x_test, y_test, q_test = load_data(valid_data_feats, valid_data_group)
        train(x_train, y_train, q_train, x_test, y_test, q_test, model_path)
        ...
        y_pred,t_results = predict(test_X, comments, model_path)
        print(len(y_pred))
        qid_idx_writer = open(f'{base_path}/data/test/output.tsv','w')
        for i in y_pred:
            qid_idx_writer.write(f'{i}\n')
        train_end = datetime.now()
        consume_time = (train_end - train_start).seconds
        print("consume time : {}".format(consume_time))
        ...
        # ndcg
        test_path = base_path + '/data/valid/valid_f.txt'#评估测试数据的平均ndcg
