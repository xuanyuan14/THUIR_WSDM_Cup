                    ...
                    if len(line_list) == 3:  # new queryr
                        ...
                        try:
                            src_input, segment, src_padding_mask = process_data(self.cur_query, title, content,
                                                                                self.max_seq_len)
                            tmp_buffer.append([qid, src_input, segment, src_padding_mask, int(click_label)])
...
# class TrainDataset_list(IterableDataset):
#     def __init__(self, directory_path, buffer_size=100000, max_seq_len=128):
#         super().__init__()
#         self.directory_path = directory_path
#         self.buffer_size = buffer_size
...
class TestDataset_swh(Dataset):
    
    def __init__(self, fpath, max_seq_len, data_type, buffer_size=30000000):
        super().__init__()
        self.max_seq_len = max_seq_len
        self.buffer_size = buffer_size
...
class TestDataset_swh2(Dataset):
    
    def __init__(self,buffer,fpath, max_seq_len, data_type, buffer_size=30000000):
        super().__init__()
        self.max_seq_len = max_seq_len
        self.buffer_size = buffer_size
